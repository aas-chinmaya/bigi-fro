...
import { useMoneyReceiptQuery } from "../../hooks/use-money-receipt-query";
import { useMoneyReceiptActions } from "../../hooks/use-money-receipt-actions";
...
interface MoneyReceiptEditFormProps {
  id: string;
}
...
export default function MoneyReceiptEditForm({
  id,
}: MoneyReceiptEditFormProps) {
  ...
  const {
    ...
    error: queryError,
  } = useMoneyReceiptQuery(undefined, id);

  const { updateMoneyReceipt } = useMoneyReceiptActions();
  ...
  const {
    ...
    resolver: zodResolver(moneyReceiptFormSchema),
    mode: "onSubmit",
    ...
    reset(
      {
        ...
        customerId:
          moneyReceipt.customerId ?? "",

        customerName:
          moneyReceipt.customerName ?? "",

        receiptType:
          moneyReceipt.receiptType,

        amount:
          Number(moneyReceipt.amount ?? 0),

        paymentMethod:
          moneyReceipt.paymentMethod,
  ...
  const customerLabel = selectedCustomer
    ...
    : clean(moneyReceipt?.customerName);
  ...
  const onSubmit = async (
    ...
    try {
      ...
      if (
        ...
        notify.success(
          "Money receipt updated successfully",
        );
      ...
      notify.error(
        "Unable to update money receipt. Please try again.",
      );
    }
  };
