...
// import { useBusinessesByUser } from "@/modules/business/hooks/useBusinessesByUser";
...
// import { useMemo, useState, useEffect, type ClipboardEvent } from "react";
// import { useRouter } from "next/navigation";
// import { useForm } from "react-hook-form";
// import { zodResolver } from "@hookform/resolvers/zod";
// import {
//   BadgeDollarSign, CalendarDays, Check, FileText, IndianRupee, Loader2,
//   MessageSquareText, NotebookText, Receipt, Search, ShieldCheck, UserRound, X,
// } from "lucide-react";
...
// import {
//   paymentReceiptFormSchema, todayDateInputValue, sanitizeInput,
//   type PaymentReceiptFormValues,
// } from "../../validation/payment-receipt.validation";
...
// function getCustomerLabel(c: Customer): string {
//   return clean((c as any).name) || clean((c as any).customerName) || clean((c as any).companyName) || String(c.id);
// }
...
// const DEFAULT_VALUES: PaymentReceiptFormValues = {
//   businessId: "", branchId: "", receiptDate: todayDateInputValue(),
//   financialYear: getFinancialYearOptions()[10], customerId: "", customerName: "",
//   customerPhone: "", customerGSTIN: "", paymentMethod: "CASH", amount: 0,
//   remarks: "", notes: "", createdBy: "",
// };
...
//   const { createPaymentReceipt } = usePaymentReceiptActions();
//   const { customers, loading: customersLoading } = useCustomers();
//   const { businesses, loading: businessesLoading } = useBusinessesByUser();
...
//   // --- dynamic business/branch/createdBy wiring ---
//   const businessId = watch("businessId");
//   const selectedBusiness = (businesses ?? []).find((b: any) => String(b.id) === String(businessId));
//   const branches = selectedBusiness?.branches ?? [];

//   useEffect(() => {
//     if (!businesses?.length || businessId) return;
//     const first = businesses[0] as any;
//     setValue("businessId", first.id, { shouldValidate: true, shouldDirty: true });
//     setValue("createdBy", first.createdBy, { shouldValidate: true, shouldDirty: true });
//     if (first.branches?.[0]?.id) {
//       setValue("branchId", first.branches[0].id, { shouldValidate: true, shouldDirty: true });
//     }
//   }, [businesses, businessId, setValue]);

//   const onBusinessChange = (id: string) => {
//     const biz = (businesses ?? []).find((b: any) => String(b.id) === id) as any;
//     setValue("businessId", id, { shouldValidate: true, shouldDirty: true });
//     setValue("createdBy", biz?.createdBy ?? "", { shouldValidate: true, shouldDirty: true });
//     setValue("branchId", biz?.branches?.[0]?.id ?? "", { shouldValidate: true, shouldDirty: true });
//   };
//   // --------------------------------------------------
...
//       [(c as any).name, (c as any).customerName, (c as any).companyName, (c as any).mobile, (c as any).email]
...
//     setValue("customerId", String(customer.id), { shouldDirty: true, shouldValidate: true });
//     setValue("customerName", customerName, { shouldDirty: true, shouldValidate: true });
//     setValue("customerPhone", (customer as any).mobile ?? "", { shouldDirty: true, shouldValidate: true });
//     setValue("customerGSTIN", (customer as any).gstin ?? "", { shouldDirty: true, shouldValidate: true });
//     setCustomerSelectQuery(customerName);
//     setCustomerSearchFocused(false);
//   };
...
//     setValue(field, sanitizeInput(text), { shouldDirty: true, shouldValidate: true });
//   };
...
//         return;
//       }
//       notify.error((result.payload as string) || "Failed to create payment receipt");
//     } catch {
//       notify.error("Unable to create payment receipt. Please try again.");
//     }
//   };

//   return (
//     <form className="mx-auto f-full w-full space-y-4">
//       <input {...register("createdBy")} type="hidden" />
//       <input {...register("invoiceId")} type="hidden" />
...
//             <Receipt className="mt-1 size-6 shrink-0 text-primary" />
//             <div>
//               <CardTitle className="text-base font-semibold">Create Payment Receipt</CardTitle>
//               <p className="text-sm text-muted-foreground">Record a customer payment</p>
//             </div>
//           </div>
//         </CardHeader>
...
//                 </Label>
//                 <Select value={watch("businessId") || undefined} onValueChange={onBusinessChange}>
//                   <SelectTrigger className="h-10 border-slate-200 bg-slate-50">
//                     <SelectValue placeholder={businessesLoading ? "Loading..." : "Select business"} />
//                   </SelectTrigger>
//                   <SelectContent className="max-h-[220px] overflow-y-auto rounded-xl">
//                     {(businesses ?? []).map((b: any) => (
//                       <SelectItem key={b.id} value={b.id}>{b.displayName || b.tradeName}</SelectItem>
//                     ))}
//                   </SelectContent>
//                 </Select>
//               </div>
...
//                   onValueChange={(v) => setValue("branchId", v, { shouldValidate: true, shouldDirty: true })}
...
//                   <SelectContent className="max-h-[220px] overflow-y-auto rounded-xl">
//                     {branches.map((br: any) => (
//                       <SelectItem key={br.id} value={br.id}>{br.branchName}</SelectItem>
//                     ))}
//                   </SelectContent>
//                 </Select>
//               </div>
...
//               <div className="space-y-2">
//                 <Label className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
//                   <UserRound className="size-4 text-slate-400" /> Customer <span className="text-red-500">*</span>
//                 </Label>
//                 <div className="relative">
//                   <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" />
//                   <Input
...
//                     onFocus={() => setCustomerSearchFocused(true)}
//                     onBlur={() => setTimeout(() => setCustomerSearchFocused(false), 140)}
//                     onChange={(e) => { setCustomerSelectQuery(e.target.value); setCustomerSearchFocused(true); }}
//                     placeholder="Search customer"
//                     className="h-10 border-slate-200 bg-slate-50 pl-9 pr-9"
//                   />
//                   {customerSelectQuery && (
//                     <button type="button" onClick={() => setCustomerSelectQuery("")}
//                       className="absolute right-2 top-1/2 -translate-y-1/2 rounded-sm p-1 text-slate-500 hover:bg-slate-200">
//                       <X className="size-4" />
//                     </button>
//                   )}
...
//                               className={`flex w-full items-start gap-3 rounded-lg border p-2 text-left transition-colors ${isActive ? "border-primary/30 bg-primary/5" : "border-slate-200 bg-white hover:bg-slate-100"}`}>
//                               <Badge text={name} />
//                               <span className="min-w-0 flex-1">
//                                 <span className="flex items-center justify-between gap-2">
//                                   <span className="truncate text-sm font-semibold text-slate-800">{name}</span>
//                                   {isActive && <Check className="size-4 shrink-0 text-primary" />}
//                                 </span>
//                                 <span className="mt-1 block truncate text-[11px] text-slate-500">{(customer as any).email || "No email"}</span>
//                                 <span className="mt-1 block truncate text-[11px] text-slate-500">{(customer as any).mobile || "No phone"}</span>
//                               </span>
//                             </button>
//                           );
//                         })
...
//               <div className="space-y-2">
//                 <Label htmlFor="receiptDate" className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
//                   <CalendarDays className="size-4 text-slate-400" /> Date <span className="text-red-500">*</span>
//                 </Label>
//                 <Input id="receiptDate" type="datetime-local" {...register("receiptDate")}
//                   aria-invalid={Boolean(errors.receiptDate)} className="h-10 border-slate-200 bg-slate-50" />
//               </div>

//               <div className="space-y-2">
//                 <Label className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
//                   <ShieldCheck className="size-4 text-slate-400" /> Financial Year <span className="text-red-500">*</span>
//                 </Label>
//                 <Select value={watch("financialYear") || undefined}
//                   onValueChange={(v) => setValue("financialYear", v, { shouldValidate: true, shouldDirty: true })}>
//                   <SelectTrigger className="h-10 border-slate-200 bg-slate-50"><SelectValue placeholder="Select year" /></SelectTrigger>
//                   <SelectContent className="max-h-[220px] overflow-y-auto rounded-xl">
//                     {financialYearOptions.map((y) => <SelectItem key={y} value={y}>{y}</SelectItem>)}
//                   </SelectContent>
//                 </Select>
//               </div>
...
//               <div className="space-y-2">
//                 <Label className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
//                   <BadgeDollarSign className="size-4 text-slate-400" /> Payment Method <span className="text-red-500">*</span>
//                 </Label>
//                 <Select value={watch("paymentMethod")}
//                   onValueChange={(v) => setValue("paymentMethod", v as PaymentReceiptFormValues["paymentMethod"], { shouldValidate: true, shouldDirty: true })}>
//                   <SelectTrigger className="h-10 border-slate-200 bg-slate-50"><SelectValue placeholder="Select payment method" /></SelectTrigger>
//                   <SelectContent className="max-h-[220px] overflow-y-auto rounded-xl">
//                     <SelectItem value="CASH">Cash</SelectItem>
//                     <SelectItem value="UPI">UPI</SelectItem>
//                     <SelectItem value="CARD">Card</SelectItem>
...
//               <div className="space-y-2">
//                 <Label htmlFor="amount" className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
//                   <IndianRupee className="size-4 text-slate-400" /> Amount <span className="text-red-500">*</span>
//                 </Label>
//                 <div className="relative">
//                   <IndianRupee className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" />
//                   <Input id="amount" type="number" min="0.01" step="0.01" placeholder="0.00"
//                     {...register("amount", { valueAsNumber: true })}
//                     aria-invalid={Boolean(errors.amount)} className="h-10 border-slate-200 bg-slate-50 pl-9" />
//                 </div>
//               </div>
//             </div>
...
//               <div className="space-y-2">
//                 <Label className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
//                   <FileText className="size-4 text-slate-400" /> Invoice
//                 </Label>
//                 <div className="relative">
//                   <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" />
//                   <Input value={invoiceQuery} onFocus={() => setInvoiceFocused(true)}
//                     onBlur={() => setTimeout(() => setInvoiceFocused(false), 140)}
//                     onChange={(e) => setInvoiceQuery(e.target.value)}
//                     placeholder="Search invoice by id or number"
//                     className="h-10 border-slate-200 bg-slate-50 pl-9 pr-9" />
...
//                           <div className="min-w-0 flex-1">
//                             <div className="truncate text-sm font-semibold text-slate-800">{inv.invoiceNumber || inv.id}</div>
//                             <div className="mt-1 truncate text-[11px] text-slate-500">Customer: {inv.customerName || inv.customerId}</div>
//                           </div>
//                         </button>
//                       ))}
//                     </div>
...
//               <div className="space-y-2">
//                 <Label htmlFor="remarks" className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
//                   <MessageSquareText className="size-4 text-slate-400" /> Remarks
//                 </Label>
//                 <Textarea id="remarks" placeholder="Optional remarks..." rows={3}
//                   className="min-h-[90px] resize-y border-slate-200 bg-slate-50"
//                   {...register("remarks")} onPaste={(e) => handleSanitizePaste(e, "remarks")} />
//               </div>

//               <div className="space-y-2">
//                 <Label htmlFor="notes" className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">
//                   <NotebookText className="size-4 text-slate-400" /> Notes
//                 </Label>
//                 <Textarea id="notes" placeholder="Optional internal notes..." rows={3}
//                   className="min-h-[90px] resize-y border-slate-200 bg-slate-50"
//                   {...register("notes")} onPaste={(e) => handleSanitizePaste(e, "notes")} />
//               </div>
//             </div>
//           </div>
